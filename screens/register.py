...
class register_page(tk.Frame):
    ...
    def handle_signup(self):
        ...
        if (len(self.password) < 8 or not any(char.isdigit() for char in self.password)):
            messagebox.showerror('Password validation Error!',
                                 "Password should include at least 1 number and should be longer than 8 !")
            return
        # check if passwords are matched
        elif (self.password != self.confirm_password):
            messagebox.showerror('Password valdiation Error!',
        ...
        user = controller1.signUpAndLogin(
            name=self.name, secret=self.password, email=self.email, phone=self.phone)
        if user:
            controller1.change_frame('login')  # check if user is created
    ...
    def __init__(self, container, controller):
        ...
        self.email_entry = Entry(self, width=60, font=(
            'Helvetica', 15), highlightthickness=0)
        self.email_entry.config(borderwidth=0)
        self.email_entry.place(relx=.16, rely=.235)

        self.phone_entry = Entry(self, width=60, font=(
            'Helvetica', 15), highlightthickness=0)
        ...
        self.name_entry = Entry(self, width=60, font=(
            'Helvetica', 15), highlightthickness=0)
        ...
        self.password_entry = Entry(self, width=60, font=(
            'Helvetica', 15), highlightthickness=0)
        ...
        self.confirm_password_entry = Entry(
            self, width=60, font=('Helvetica', 15), highlightthickness=0)
